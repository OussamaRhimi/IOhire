...
export default factories.createCoreController('api::department.department', ({ strapi }) => ({
  async hrList(ctx) {
    const rows = (await strapi.entityService.findMany('api::department.department', {
  ...
  async hrCreate(ctx) {
    ...
    const duplicate = (await strapi.entityService.findMany('api::department.department', {
      fields: ['name'] as any,
      filters: { name: { $eqi: name } } as any,
      limit: 1,
    })) as any[];
    if (duplicate?.length) return ctx.badRequest('A department with this name already exists.');
  ...
  async hrUpdate(ctx) {
    ...
    if (!id) return ctx.badRequest('Invalid department id.');

    const body = (ctx.request as any).body ?? {};
    const name = normalizeName(body?.name ?? body?.data?.name);
    if (!name) return ctx.badRequest('name is required.');

    const existing = await strapi.entityService.findOne('api::department.department', id, {
      fields: ['name', 'documentId'] as any,
    });
